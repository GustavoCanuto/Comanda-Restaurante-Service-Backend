...
public class TratadorDeErros {
	...
	// ex: http://localhost:8080/id/15000
	...
	public ResponseEntity<List<ErrorEnumResponse>> tratarErro400(MethodArgumentNotValidException ex) {
		var erros = ex.getFieldErrors();

		return ResponseEntity.badRequest().body(erros.stream().map(ErrorEnumResponse::new).toList());
	}

	@JsonInclude(JsonInclude.Include.NON_NULL)
	private record ErrorEnumResponse(String campo, String mensagem, List<String> opcoes) {
		public ErrorEnumResponse(FieldError erro) {
			this(erro.getField(), erro.getDefaultMessage(), null);
		}
	}
	...
	public ResponseEntity<ErrorEnumResponse> handleEnumInvalidoException(EnumInvalidoException ex) {
		...
		ErrorEnumResponse errorResponse = new ErrorEnumResponse(null, mensagem, opcoes);
